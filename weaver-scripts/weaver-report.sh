...
function setup_envs()
{
  PATH_FEATURES=$(cat weaver-report-config.yml | grep path_to_features | awk '{print $2}')
  PATH_DOC=$(cat weaver-report-config.yml | grep path_to_all_scenarios_document_txt | awk '{print $2}')
...
function set_collapse_feature_html()
{
  ...
  SCENARIOS_TOTAL_DOCUMENT=$(echo $SCENARIOS_TOTAL_DOCUMENT | tr -d ' ')
  echo  "<p class='m-t-30 f-w-600'>"$FEATURE "("$SCENARIOS_TOTAL"/"$SCENARIOS_TOTAL_DOCUMENT")""<span class='pull-right'>"$PERCENTAGE"%</span></p>" >> $FILE_HTML
  echo      "<div class='progress '>" >> $FILE_HTML
  if [ $PERCENTAGE -lt '70' ]  || [ $PERCENTAGE -eq '0' ]
  then
    echo "<div role='progressbar' style='width: $PERCENTAGE%; height:8px;' class='progress-bar bg-danger wow animated progress-animated'> <span class='sr-only'>$PERCENTAGE% Complete</span> </div>" >> $FILE_HTML
  elif [ $PERCENTAGE -gt '70' ] || [ $PERCENTAGE -eq '70'] 
  then
    echo "<div role='progressbar' style='width: $PERCENTAGE%; height:8px;' class='progress-bar bg-info wow animated progress-animated'> <span class='sr-only'>$PERCENTAGE% Complete</span> </div>" >> $FILE_HTML
  elif [ $PERCENTAGE -eq '100' ] 
  then
    echo "<div role='progressbar' style='width: $PERCENTAGE%; height:8px;' class='progress-bar bg-sucess wow animated progress-animated'> <span class='sr-only'>$PERCENTAGE% Complete</span> </div>" >> $FILE_HTML
  fi
...
function get_all_datas_functional()
{
  COBERTURA=0
  SCENARIOS_TOTAL=0
  SCENARIOS_TOTAL_PROJECT=0
  SCENARIOS_TOTAL_DOC=0

  ls $PATH_FEATURES | grep '.feature' >> features.txt
  i=1
  while read LINE
  do

    FEATURE=$(cat $PATH_FEATURES$LINE   | grep Funcionalidade: | awk '{print $2}')
    SCENARIOS_TOTAL=$(cat $PATH_FEATURES$LINE  | grep @$PLATFORM_NAME -A1 | grep Cenário | wc -l)
    # cat $PATH_FEATURES$LINE  | grep @$PLATFORM_NAME -A1 | grep Cenário
    # cat $PATH_FEATURES$LINE  | grep @$PLATFORM_NAME -A1 | grep Cenário | wc -l 
    SCENARIOS_TOTAL_PROJECT=$(($SCENARIOS_TOTAL_PROJECT+$SCENARIOS_TOTAL))
    cat $PATH_FEATURES$LINE  | grep @$PLATFORM_NAME -A1 | grep Cenário >> scenarios.txt
    SCENARIOS_TOTAL_DOCUMENT=$(cat $PATH_DOC | grep $FEATURE | wc -l)
    cat $PATH_DOC | grep $FEATURE | awk -F"\t" '{print $3}' >> scenario_doc.txt
    PERCENTAGE=$((($SCENARIOS_TOTAL*100)/$SCENARIOS_TOTAL_DOCUMENT))
    # echo $FEATURE":"$PERCENTAGE"%"
    ...
    while read L 
    do 
      SCENARIO_NAME=$(cat scenarios.txt | grep "$L")
      if [ -z "$SCENARIO_NAME" ]; then
        set_scenarios_not_implemented_to_collapse_html
        # echo -e "\033[031;1m$L\033[m" 
        j=$(($j+1))
      else
        set_scenarios_implemented_to_collapse_html
        j=$(($j+1))
      fi
    done < scenario_doc.txt

    rm -rf scenarios.txt
    rm -rf scenario_doc.txt
    i=$(($i+1))
    set_end_html
  done < features.txt
  SCENARIOS_TOTAL_DOC=$(cat $PATH_DOC | wc -l)
  COBERTURA=$(echo $SCENARIOS_TOTAL_PROJECT 100 $SCENARIOS_TOTAL_DOC | awk '{print ($1*$2) / $3}')
  COBERTURA=$(printf %.2f $COBERTURA)
  SCENARIOS_TOTAL_DOC=$(echo $SCENARIOS_TOTAL_DOC | tr -d ' ')


  # echo 'COBERTURA DE TESTES FUNCIONAIS AUTOMATIZADOS: '$COBERTURA"%"
  COBERTURA=$(echo $COBERTURA | tr "," ".")
  ALL_METRICS=$ALL_METRICS" "$COBERTURA
...
function set_metrics_in_template()
{
  ...
  cat ../weaver-template/index.html | sed -e "s|FUNCTIONAL_PERCENTAGE|${COBERTURA}|" > report_tests.html
  cat report_tests.html | sed -e "s|FUNCTIONAL_SCENARIOS_NUMBER|${SCENARIOS_TOTAL_DOC}|" > report_tests_1.html
  ...
  cat ../weaver-template/js/lib/morris-chart/morris-init.js | sed -e "s|INFO_BUGS_SPRINT|${FILE_BUGS}|" > report_bugs.js
  ...
  cat report_bugs.js | sed -e "s|FLAGGED_SPRINT|${FLAGGED_CURRENT}|" > ../weaver-template/js/lib/morris-chart/morris-init-$PLATFORM_NAME.js
...
{
  cat ../weaver-template/chart-morris.html >> chart-morris-$PLATFORM_NAME.html
...
function generate_weaver_report
{
  ...
  cp -r ../weaver-template/ $REPORT_NAME/
